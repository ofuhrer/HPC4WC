    ...
    for (std::size_t k = 0; k < zMax; ++k) {
      ...
      for (std::size_t j = yMin; j < yMax; ++j) {
        for (std::size_t i = xMin; i < xMax; ++i) {
          ...
          double laplap = 20 * inField[ijk] -
                          8 * (inField[im1jk] + inField[ip1jk] +
                               inField[ijm1k] + inField[ijp1k]) +
                          2 * (inField[im1jm1k] + inField[ip1jm1k] +
                               inField[im1jp1k] + inField[ip1jp1k]) +
                          1 * (inField[im2jk] + inField[ip2jk] +
                               inField[ijm2k] + inField[ijp2k]);
