...
void updateHalo(double * inField,
								int32_t xsize,
								int32_t ysize,
								int32_t zsize,
								int32_t halosize )
...
// deprecated older version of the function, here for ... documentation reasons?
void apply_diffusion(Storage3D<double>& inField,
										 Storage3D<double>& outField,
										 double alpha, unsigned numIter,
										 int x, int y, int z, int halo)
{
...
// overloaded function with cstd arrays
void apply_diffusion(double *inField,
										 double *outField,
										 double alpha, unsigned numIter,
										 int x, int y, int z, int halo)
{
      ...
      //   for(std::size_t i = xMin - 1; i < xMax + 1; ++i) {
        ...
        for(std::size_t i = xMin; i < xMax; ++i) {
					...
					std::size_t ijp2k = i + ((j+2) * xsize) + k*(xsize*ysize);
 					...
 					double laplap =
						20*inField[ijk] -
						8*( inField[im1jk] + inField[ip1jk] + inField[ijm1k] + inField[ijp1k] ) +
						2*( inField[im1jm1k] + inField[ip1jm1k] + inField[im1jp1k] + inField[ip1jp1k] ) +
						1*( inField[im2jk] + inField[ip2jk] + inField[ijm2k] + inField[ijp2k] );
  ...
  output_3D.initialize();
	...
	for(std::size_t k = zsize / 4.0; k < 3 * zsize / 4.0; ++k) {
		for(std::size_t j = nHalo + xsize / 4.; j < nHalo + 3. / 4. * xsize; ++j) {
			for(std::size_t i = nHalo + xsize / 4.; i < nHalo + 3. / 4. * xsize; ++i) {
