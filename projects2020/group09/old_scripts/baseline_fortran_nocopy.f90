...
program main
    ...
    integer, parameter :: lblocking = 4 ! 1; blocking with blocks as specified below, 0: no blocking
    ...
    do ivar = 1, nvar
        if (lblocking == 1) then
            ! TODO: change this so that ivar and the whole array get passed
            CALL weightsum_blocking(mask(ivar,:,:,:), weights(ivar,:,:,:))
            CALL nanmean_blocking(in_field(:,:,:,:), weights(:,:,:,:), mask(:,:,:,:), out_field(:,:,:,:),ivar)
        else if (lblocking == 2) then
            CALL weightsumnanmean_combined(in_field(:,:,:,:), mask(:,:,:,:), out_field(:,:,:,:), ivar)
        else if (lblocking == 4) then
            CALL weightsumnanmean_combined_blocking(in_field(:,:,:,:), mask(:,:,:,:), out_field(:,:,:,:), ivar)
        else
            CALL weightsum(mask(:,:,:,:), weights(:,:,:,:), ivar)
            CALL nanmean(in_field(:,:,:,:), weights(:,:,:,:), mask(:,:,:,:), out_field(:,:,:,:), ivar)
        end if
        write(*,*) 'ivar = ', ivar
    end do
...
contains
    ...
    subroutine weightsumnanmean_combined(in_field, mask, out_field, ivar)
        ...
        do i = 1+nhalo, nx-nhalo
            ...
            out_field(ivar,il,jl,kl) = tmp / max(tmpm, 1.0_wp)
        end do
        end do
        end do
    ...
    subroutine weightsum(mask, weights, ivar)
        ...
        do j = 1+nhalo, ny-nhalo
        do i = 1+nhalo, nx-nhalo
            CALL weights_stencil_loop(mask, i,j,k, weights(i,j,k,ivar),ivar)
        end do
        end do
        end do
    ...
    subroutine weightsum_blocking(mask, weights)
        ...
        do i_local = 1, blockx
            ...
            CALL weights_stencil(mask, i,j,k, weights(i,j,k))
            !write(*,*) weights(i,j,k)
        end do
        end do
        end do
    ...
    subroutine nanmean(in_field, weights, mask, out_field, ivar)
        ! loop over all points of a certain var-field and compute generic_filter for each
        implicit none
        ...
        do j = 1+nhalo, ny-nhalo
        do i = 1+nhalo, nx-nhalo
            CALL nanmean_stencil_loop(in_field, mask, i,j,k, weights(ivar,i,j,k), out_field(ivar,i,j,k), ivar)
        end do
        end do
        end do
    end subroutine nanmean

    subroutine nanmean_blocking(in_field, weights, mask, out_field, ivar)
        ! loop over all points of a certain var-field and compute generic_filter for each
        implicit none
        ...
        do i_local = 1, blockx
            ...
            !write(*,*) 'b'
            !write(*,*) weights(i,j,k)
            CALL nanmean_stencil_loop(in_field, mask, i,j,k, weights(i,j,k,ivar), out_field(i,j,k, ivar), ivar)
        end do
        end do
        end do
        end do
    ...
    subroutine nanmean_stencil_loop(v,w,i,j,k,wvalue,outvalue, ivar)

            implicit none

            real(kind=wp), intent(in) :: v(:,:,:,:), w(:,:,:,:), wvalue
            ...
            do jl = j-2,j+2
            do kl = k-2,k+2
                outvalue = outvalue + v(ivar,il,jl,kl) * w(ivar,il,jl,kl)
            end do
            end do
            end do
            outvalue = outvalue / max(wvalue, 1.0_wp)
    ...
    subroutine weights_stencil_loop(w,i,j,k,outvalue,ivar)
            ...
            do jl = j-2,j+2
            do kl = k-2,k+2
                outvalue = outvalue + w(ivar,il,jl,kl)
            end do
            end do
            end do
    ...
    subroutine weights_stencil(w, i,j,k, outvalue)
            ...
            outvalue = &
            w(i+1,j+1,k+5) + w(i+1,j+2,k+5) + w(i+1,j+3,k+5) + w(i+1,j+4,k+5) + w(i+1,j+5,k+5) + &
            w(i+2,j+1,k+5) + w(i+2,j+2,k+5) + w(i+2,j+3,k+5) + w(i+2,j+4,k+5) + w(i+2,j+5,k+5) + &
            w(i+3,j+1,k+5) + w(i+3,j+2,k+5) + w(i+3,j+3,k+5) + w(i+3,j+4,k+5) + w(i+3,j+5,k+5) + &
            w(i+4,j+1,k+5) + w(i+4,j+2,k+5) + w(i+4,j+3,k+5) + w(i+4,j+4,k+5) + w(i+4,j+5,k+5) + &
            w(i+5,j+1,k+5) + w(i+5,j+2,k+5) + w(i+5,j+3,k+5) + w(i+5,j+4,k+5) + w(i+5,j+5,k+5) + &

            w(i+1,j+1,k+4) + w(i+1,j+2,k+4) + w(i+1,j+3,k+4) + w(i+1,j+4,k+4) + w(i+1,j+5,k+4) + &
            w(i+2,j+1,k+4) + w(i+2,j+2,k+4) + w(i+2,j+3,k+4) + w(i+2,j+4,k+4) + w(i+2,j+5,k+4) + &
            w(i+3,j+1,k+4) + w(i+3,j+2,k+4) + w(i+3,j+3,k+4) + w(i+3,j+4,k+4) + w(i+3,j+5,k+4) + &
            w(i+4,j+1,k+4) + w(i+4,j+2,k+4) + w(i+4,j+3,k+4) + w(i+4,j+4,k+4) + w(i+4,j+5,k+4) + &
            w(i+5,j+1,k+4) + w(i+5,j+2,k+4) + w(i+5,j+3,k+4) + w(i+5,j+4,k+4) + w(i+5,j+5,k+4) + &

            w(i+1,j+1,k+3) + w(i+1,j+2,k+3) + w(i+1,j+3,k+3) + w(i+1,j+4,k+3) + w(i+1,j+5,k+3) + &
            w(i+2,j+1,k+3) + w(i+2,j+2,k+3) + w(i+2,j+3,k+3) + w(i+2,j+4,k+3) + w(i+2,j+5,k+3) + &
            w(i+3,j+1,k+3) + w(i+3,j+2,k+3) + w(i+3,j+4,k+3) + w(i+3,j+5,k+3) + w(i+3,j+3,k+3) + & ! w point itself!
            w(i+4,j+1,k+3) + w(i+4,j+2,k+3) + w(i+4,j+3,k+3) + w(i+4,j+4,k+3) + w(i+4,j+5,k+3) + &
            w(i+5,j+1,k+3) + w(i+5,j+2,k+3) + w(i+5,j+3,k+3) + w(i+5,j+4,k+3) + w(i+5,j+5,k+3) + &

            w(i+1,j+1,k+2) + w(i+1,j+2,k+2) + w(i+1,j+3,k+2) + w(i+1,j+4,k+2) + w(i+1,j+5,k+2) + &
            w(i+2,j+1,k+2) + w(i+2,j+2,k+2) + w(i+2,j+3,k+2) + w(i+2,j+4,k+2) + w(i+2,j+5,k+2) + &
            w(i+3,j+1,k+2) + w(i+3,j+2,k+2) + w(i+3,j+3,k+2) + w(i+3,j+4,k+2) + w(i+3,j+5,k+2) + &
            w(i+4,j+1,k+2) + w(i+4,j+2,k+2) + w(i+4,j+3,k+2) + w(i+4,j+4,k+2) + w(i+4,j+5,k+2) + &
            w(i+5,j+1,k+2) + w(i+5,j+2,k+2) + w(i+5,j+3,k+2) + w(i+5,j+4,k+2) + w(i+5,j+5,k+2) + &

            w(i+1,j+1,k+1) + w(i+1,j+2,k+1) + w(i+1,j+3,k+1) + w(i+1,j+4,k+1) + w(i+1,j+5,k+1) + &
            w(i+2,j+1,k+1) + w(i+2,j+2,k+1) + w(i+2,j+3,k+1) + w(i+2,j+4,k+1) + w(i+2,j+5,k+1) + &
            w(i+3,j+1,k+1) + w(i+3,j+2,k+1) + w(i+3,j+3,k+1) + w(i+3,j+4,k+1) + w(i+3,j+5,k+1) + &
            w(i+4,j+1,k+1) + w(i+4,j+2,k+1) + w(i+4,j+3,k+1) + w(i+4,j+4,k+1) + w(i+4,j+5,k+1) + &
            w(i+5,j+1,k+1) + w(i+5,j+2,k+1) + w(i+5,j+3,k+1) + w(i+5,j+4,k+1) + w(i+5,j+5,k+1)
