...
class Partitioner:
    ...
    def __init__(self, comm, domain, num_halo, periodic=(True, True)):
        assert len(domain) == 3, "Must specify a three-dimensional domain"
        ...
        assert domain[1] >= size[0] and domain[2] >= size[1], (
            "Domain is too small for number of ranks"
        )
    ...
    def scatter(self, field, root=0):
        ...
        if self.__rank == root:
            ...
            for rank in range(self.__num_ranks):
                ...
                sendbuf[rank, :, : j_end - j_start, : i_end - i_start] = field[
    ...
    def gather(self, field, root=0):
        ...
        if self.__rank == root or root == -1:
            ...
            for rank in range(self.__num_ranks):
                j_start, i_start, j_end, i_end = self.__domains[rank]
                global_field[:, j_start:j_end, i_start:i_end] = recvbuf[
                    rank, :, : j_end - j_start, : i_end - i_start
                ]
        return global_field
    ...
    def compute_domain(self):
        ...
        return [
            self.__domain[0] + self.__num_halo,
            self.__domain[1] + self.__num_halo,
            self.__domain[2] - self.__num_halo,
            self.__domain[3] - self.__num_halo,
        ]
    ...
    def __setup_domain(self, shape, num_halo):
        ...
        for rank in range(self.__num_ranks):
            ...
            domains += [
                [
                    pos_y[pos[0]] - num_halo,
                    pos_x[pos[1]] - num_halo,
                    pos_y[pos[0] + 1] + num_halo,
                    pos_x[pos[1] + 1] + num_halo,
                ]
            ]
            shapes += [
                [size_z, domains[rank][2] - domains[rank][0], domains[rank][3] - domains[rank][1]]
            ]
