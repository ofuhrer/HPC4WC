...
__global__ void finalize(float *gpu_field, float *tmp_field, float *tmp2_field, float alpha) {
    unsigned int w = gpu_grid.nx;
    unsigned int h = gpu_grid.ny;
    unsigned int nh = gpu_grid.num_halo;
    ...
    if (x >= nh && x < nh + w &&
	y >= nh && y < nh + h) {
	tmp2_field[A3D(x, y, z, w + 2 * nh, h + 2 * nh)] = -4.0 * tmp_field[A3D(x, y, z, w + 2 * nh, h + 2 * nh)]
...
void stencil(float *gpu_field, float *tmp_field, float *tmp2_field,
	...
	finalize<<<block_grid_2, thread_grid_2>>>(gpu_field, tmp_field, tmp2_field, alpha);
	halo_horizontal<<<block_grid_h, thread_grid_h>>>(gpu_field);
	halo_vertical<<<block_grid_v, thread_grid_v>>>(gpu_field);
    }
}
