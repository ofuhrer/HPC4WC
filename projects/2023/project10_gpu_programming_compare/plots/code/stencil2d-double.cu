...
__global__ void halo_vertical(double *field) {
    unsigned int w = gpu_grid.nx;
    unsigned int h = gpu_grid.ny;
    unsigned int nh = gpu_grid.num_halo;
...
__global__ void laplace(double *gpu_field, double *tmp_field) {
    unsigned int w = gpu_grid.nx;
    unsigned int h = gpu_grid.ny;
    unsigned int nh = gpu_grid.num_halo;
...
__global__ void finalize(double *gpu_field, double *tmp_field, double alpha) {
    unsigned int w = gpu_grid.nx;
    unsigned int h = gpu_grid.ny;
    unsigned int nh = gpu_grid.num_halo;
    ...
    if (x >= nh && x < nh + w &&
	y >= nh && y < nh + h) {
	double laplap = -4.0 * tmp_field[A3D(x, y, z, w + 2 * nh, h + 2 * nh)]
...
void stencil(double *gpu_field, double *tmp_field,
	     double alpha, double num_iter, grid_info grid, cudaStream_t *stream) {
    ...
    dim3 block_grid_h = dim3(1, 1, d);
    dim3 thread_grid_h = dim3(nh, h, 1);

    dim3 block_grid_v = dim3(1, 1, d);
    dim3 thread_grid_v = dim3(w + 2 * nh, nh, 1);

    void * params1[3] = {&gpu_field, &tmp_field};
    void * params2[3] = {&gpu_field, &tmp_field, &alpha};
    void * params3[] = {&gpu_field};

    gpuErrchk(cudaLaunchKernel((void *) halo_horizontal, block_grid_h, thread_grid_h, params3, 0, *stream));
    gpuErrchk(cudaLaunchKernel((void *) halo_vertical, block_grid_v, thread_grid_v, params3, 0, *stream));
    for (unsigned int iter = 0; iter < num_iter; ++iter) {
	gpuErrchk(cudaLaunchKernel((void *) laplace, block_grid_1, thread_grid_1, params1, 0, *stream));
	gpuErrchk(cudaLaunchKernel((void *) finalize, block_grid_2, thread_grid_2, params2, 0, *stream));
	gpuErrchk(cudaLaunchKernel((void *) halo_horizontal, block_grid_h, thread_grid_h, params3, 0, *stream));
	gpuErrchk(cudaLaunchKernel((void *) halo_vertical, block_grid_v, thread_grid_v, params3, 0, *stream));
    }
}
...
__global__ void field_init (double *in_field) {
    unsigned int w = gpu_grid.nx;
    unsigned int h = gpu_grid.ny;
...
void setup (double **in_field, double **out_field, double **tmp_field, double **gpu_field,
    ...
    // CPU allocations
    *in_field = (double *) malloc((w + 2 * nh) * (h + 2 * nh) * d * sizeof(double));
    *out_field = (double *) malloc((w + 2 * nh) * (h + 2 * nh) * d * sizeof(double));

    // GPU allocations
    gpuErrchk(cudaMalloc((void **) gpu_field, (w + 2 * nh) * (h + 2 * nh) * d * sizeof(double)));
    gpuErrchk(cudaMalloc((void **) tmp_field, (w + 2 * nh) * (h + 2 * nh) * d * sizeof(double)));
...
void cleanup (double **in_field, double **out_field, double **tmp_field, double **gpu_field) {
    free(*in_field);
    free(*out_field);
...
int main (int argc, char *argv[]) {
	...
	gpuErrchk(cudaMemcpy(out_field, gpu_field,
			     ...
			     grid.nz * sizeof(double), cudaMemcpyDeviceToHost));
