...
__global__ void laplace(float *gpu_field, float *tmp_field, unsigned int extend) {
    unsigned int w = gpu_grid.nx;
    unsigned int h = gpu_grid.ny;
    unsigned int nh = gpu_grid.num_halo;

    unsigned int x = threadIdx.x + blockIdx.x * blockDim.x + nh - extend;
    unsigned int y = threadIdx.y + blockIdx.y * blockDim.y + nh - extend;
    unsigned int z = blockIdx.z;

    // Check bounds
    if (x >= nh - extend && x < nh + w + extend &&
	y >= nh - extend && y < nh + h + extend) {
...
__global__ void finalize(float *gpu_field, float *tmp2_field, float alpha) {
    unsigned int w = gpu_grid.nx;
    unsigned int h = gpu_grid.ny;
    unsigned int nh = gpu_grid.num_halo;
    ...
    if (x >= nh && x < nh + w &&
	y >= nh && y < nh + h) {
	gpu_field[A3D(x, y, z, w + 2 * nh, h + 2 * nh)] -= alpha * tmp2_field[A3D(x, y, z, w + 2 * nh, h + 2 * nh)];
    }
}
...
void stencil(float *gpu_field, float *tmp_field, float *tmp2_field,
    ...
    dim3 block_grid_1a = dim3((w + nh + THREADS - 1) / THREADS, (h + nh + THREADS - 1) / THREADS, d);
    dim3 thread_grid_1a = dim3(THREADS, THREADS, 1);

    dim3 block_grid_1b = dim3((w + THREADS - 1) / THREADS, (h + THREADS - 1) / THREADS, d);
    dim3 thread_grid_1b = dim3(THREADS, THREADS, 1);
	...
	laplace<<<block_grid_1a, thread_grid_1a>>>(gpu_field, tmp_field, 1);
	laplace<<<block_grid_1b, thread_grid_1b>>>(tmp_field, tmp2_field, 0);
	finalize<<<block_grid_2, thread_grid_2>>>(gpu_field, tmp2_field, alpha);
	halo_horizontal<<<block_grid_h, thread_grid_h>>>(gpu_field);
	halo_vertical<<<block_grid_v, thread_grid_v>>>(gpu_field);
    }
}
...
void setup (float **in_field, float **out_field, float **tmp_field, float **tmp2_field, float **gpu_field,
    ...
    *out_field = (float *) malloc((w + 2 * nh) * (h + 2 * nh) * d * sizeof(float));
    ...
    gpuErrchk(cudaMalloc((void **) tmp2_field, (w + 2 * nh) * (h + 2 * nh) * d * sizeof(float)));
...
void cleanup (float **in_field, float **out_field, float **tmp_field, float **tmp2_field, float **gpu_field) {
    free(*in_field);
    free(*out_field);
...
int main (int argc, char *argv[]) {
    ...
    float *gpu_field, *tmp_field, *tmp2_field;
	...
	// Initialize arrays
	setup(&in_field, &out_field, &tmp_field, &tmp2_field, &gpu_field,
	      &gpu_grid, &grid);
	...
	for (unsigned int i = 0; i < WARMUP_REPEATS; ++i) {
	    stencil(gpu_field, tmp_field, tmp2_field,
		    alpha, num_iter, grid);
	}
#endif
	...
	cleanup(&in_field, &out_field, &tmp_field, &tmp2_field, &gpu_field);
