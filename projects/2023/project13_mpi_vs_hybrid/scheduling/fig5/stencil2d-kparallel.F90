...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p )
        ...
        do iter = 1, num_iter
            ...
            !$omp parallel do default(none) shared(k, j, i, num_halo, out_field, in_field, alpha, tmp2_field)
            ...
            if ( iter /= num_iter ) then
                !$omp parallel do default(none) shared(k, j, i, num_halo, out_field, in_field)
    ...
    subroutine laplacian( field, lap, num_halo, extend )
        ...
        !$omp parallel do default(none) shared(lap, k, j, i, num_halo, field, extend)
