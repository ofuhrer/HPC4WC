...
!        write(*, '(a)') 'data = np.array( [ \'
        write(*, '(a, i0, a, i0, a)') 'out_', num_rank(), '[', n_threads, '] = np.array( [ \'
    end if
...
!            call laplacian( tmp1_field, tmp2_field, num_halo, extend=0 )
            ...
            !$omp parallel do private(k, j, i, tmp1_field, tmp2_field)
            do k = lbound(out_field,3), ubound(out_field,3)
                do j = lbound(in_field,2) + num_halo - 1, ubound(in_field,2) - num_halo + 1
                do i = lbound(in_field,1) + num_halo - 1, ubound(in_field,1) - num_halo + 1
                    tmp1_field(i, j) = -4._wp * in_field(i, j, k) + in_field(i-1, j, k) &
                    + in_field(i+1, j ,k) + in_field(i, j-1, k) + in_field(i, j+1, k)
                end do
                end do
                
                do j = lbound(tmp1_field,2) + num_halo, ubound(tmp1_field,2) - num_halo
                do i = lbound(tmp1_field,1) + num_halo, ubound(tmp1_field,1) - num_halo
                    tmp2_field(i, j) = -4._wp * tmp1_field(i, j) + tmp1_field(i-1, j) + &
                    tmp1_field(i+1, j) + tmp1_field(i, j-1) + tmp1_field(i, j+1)
                end do
                end do
