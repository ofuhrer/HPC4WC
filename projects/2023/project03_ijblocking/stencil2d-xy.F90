...
program main
    ...
    ! local
    ! mx, my here is the number of grid-cell of the small box considered
    integer :: nx, ny, nz, num_iter, mx, my 
    logical :: scan
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        do iter = 1, num_iter
            ...
            do k = 1, nz
                ...
                ! loop through small ij boxes
                do mj = 1, ny/my
                    do mi = 1, nx/mx

                        do j = (mj-1)*my + num_halo, mj*my + num_halo+1
                        do i = (mi-1)*mx + num_halo, mi*mx + num_halo+1
                        ...
                        do j = (mj-1)*my + num_halo + 1, mj*my + num_halo
                        do i = (mi-1)*mx + num_halo + 1, mi*mx + num_halo
    ...
    subroutine read_cmd_line_arguments()
        ...
        do while ( iarg <= num_arg )
            ...
            case ("--nz")
                ...
                iarg = iarg + 1
            case ("--mx")
                call error(iarg + 1 > num_arg, "Missing value for -mx argument")
                call get_command_argument(iarg + 1, arg_val)
                call error(arg_val(1:1) == "-", "Missing value for -mx argument")
                read(arg_val, *) mx
                iarg = iarg + 1            
            case ("--my")
                call error(iarg + 1 > num_arg, "Missing value for -my argument")
                call get_command_argument(iarg + 1, arg_val)
                call error(arg_val(1:1) == "-", "Missing value for -my argument")
                read(arg_val, *) my
                iarg = iarg + 1     
            case ("--num_iter")
