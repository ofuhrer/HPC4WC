...
Run the manual SWE solver with per-iteration halo-exchange timing.

Times only the MPI halo exchange (update_halo!) for each iteration.
The compute kernels are *not* included in the timed interval.
"""
...
@views function run_baseline(nx_global, ny_global;
    ...
    # -------------------------------------------------------------------------
    # Benchmark with per-iteration halo-exchange timing
    # -------------------------------------------------------------------------
    halo_walltimes = Float64[]
    ...
    # -------------------------------------------------------------------------
    # Write per-iteration halo-exchange wall times to CSV
    # -------------------------------------------------------------------------
    if benchmark && me == 0
        ...
        open(benchdir, "a") do io
            if !file_exists
                println(io, "solver,nprocs,topology,nx_global,ny_global,nx_local,ny_local,iteration,halo_walltime_seconds")
            end
            for idx in eachindex(halo_walltimes)
                @printf(io, "manual_halo,%d,%s,%d,%d,%d,%d,%d,%.9f\n",
                    nprocs, topology_str,
        ...
        println("All $(length(halo_walltimes)) halo-exchange wall times saved to: $benchdir")
    end
