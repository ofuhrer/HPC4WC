! Program: stencil2d-filter-inline
...
!
! Inlined variant of stencil2d-filter: fluxes held in scalars inside one
! fused flux+limit+update loop, each interior flux computed twice.
! --branchfree replaces the IF limiter with merge().
...
program main
    ...
    logical :: branchfree = .false.    ! runtime option --branchfree (merge-based limiter)
...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p )
        ...
        ! local -- all temporaries are 2D planes (array demotion)
        real (kind=wp), save, allocatable, target :: p1_field(:, :)
        real (kind=wp), save, allocatable, target :: p2_field(:, :)
        real (kind=wp), pointer :: srcp(:, :), cur(:, :), nxt(:, :), swp(:, :)
        real (kind=wp) :: alpha_eff
        real (kind=wp) :: fxw, fxe, fys, fyn
        ...
        if ( allocated(p1_field) .and. &
            any( shape(p1_field) /= (/nx + 2 * num_halo, ny + 2 * num_halo/) ) ) then
            deallocate( p1_field, p2_field )
        end if
        if ( .not. allocated(p1_field) ) then
            allocate( p1_field(nx + 2 * num_halo, ny + 2 * num_halo) )
            allocate( p2_field(nx + 2 * num_halo, ny + 2 * num_halo) )
            p1_field = 0.0_wp
            p2_field = 0.0_wp
        end if
        ...
        do iter = 1, num_iter
            ...
            ! the filter is purely horizontal, so each k-level is independent:
            ! run the whole sweep/flux/update pipeline per level on 2D planes
            do k = 1, nz
                ...
                if ( m == 1 ) then
                    ...
                    cur => p1_field
                    nxt => p2_field
                    call laplacian_2d( in_field(:, :, k), cur, num_halo, extend=m-1 )
                    do s = 2, m - 1
                        call laplacian_2d( cur, nxt, num_halo, extend=m-s )
                ...
                if ( use_limiter ) then
                    if ( branchfree ) then
                        do j = j0, j1
                        do i = i0, i1
                            fxw = srcp(i, j) - srcp(i - 1, j)
                            fxe = srcp(i + 1, j) - srcp(i, j)
                            fys = srcp(i, j) - srcp(i, j - 1)
                            fyn = srcp(i, j + 1) - srcp(i, j)
                            fxw = merge( 0.0_wp, fxw, sgn * fxw * (in_field(i, j, k) - in_field(i - 1, j, k)) <= 0.0_wp )
                            fxe = merge( 0.0_wp, fxe, sgn * fxe * (in_field(i + 1, j, k) - in_field(i, j, k)) <= 0.0_wp )
                            fys = merge( 0.0_wp, fys, sgn * fys * (in_field(i, j, k) - in_field(i, j - 1, k)) <= 0.0_wp )
                            fyn = merge( 0.0_wp, fyn, sgn * fyn * (in_field(i, j + 1, k) - in_field(i, j, k)) <= 0.0_wp )
                            out_field(i, j, k) = in_field(i, j, k) + alpha_eff * ( fxe - fxw + fyn - fys )
                        end do
                        end do
                    else
                        do j = j0, j1
                        do i = i0, i1
                            ...
                            fys = srcp(i, j) - srcp(i, j - 1)
                            fyn = srcp(i, j + 1) - srcp(i, j)
                            ! count west/south everywhere and east/north only on the
                            ! last interior column/row -> each interface counted once
                            if ( sgn * fxw * (in_field(i, j, k) - in_field(i - 1, j, k)) <= 0.0_wp ) then
                                if ( fxw /= 0.0_wp ) limited_count = limited_count + 1
                                fxw = 0.0_wp
                            end if
                            if ( sgn * fxe * (in_field(i + 1, j, k) - in_field(i, j, k)) <= 0.0_wp ) then
                                if ( i == i1 .and. fxe /= 0.0_wp ) limited_count = limited_count + 1
                                fxe = 0.0_wp
                            end if
                            if ( sgn * fys * (in_field(i, j, k) - in_field(i, j - 1, k)) <= 0.0_wp ) then
                                if ( fys /= 0.0_wp ) limited_count = limited_count + 1
                                fys = 0.0_wp
                            end if
                            if ( sgn * fyn * (in_field(i, j + 1, k) - in_field(i, j, k)) <= 0.0_wp ) then
                                if ( j == j1 .and. fyn /= 0.0_wp ) limited_count = limited_count + 1
                                fyn = 0.0_wp
                            end if
                            out_field(i, j, k) = in_field(i, j, k) + alpha_eff * ( fxe - fxw + fyn - fys )
                        end do
                        ...
                        flux_count = flux_count &
                            + int(j1 - j0 + 1, 8) * int(i1 - i0 + 2, 8) &
                            + int(j1 - j0 + 2, 8) * int(i1 - i0 + 1, 8)
                    end if
                else
                    do j = j0, j1
                    do i = i0, i1
                ...
                ! per-level copy-back is safe: the operator has no vertical coupling
    ...
    ! 2D Laplacian on a single k-level plane (2nd-order centered)
