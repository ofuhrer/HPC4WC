...
!
! 4th-order diffusion in flux form with the monotonic flux limiter of
! Xue (2000) Eq. 10 (--limiter). Full-size 3D flux arrays: the naive
! starting point of the optimization ladder.
...
program main
    ...
    integer :: num_halo = 2      ! halo width (runtime option --num_halo)
...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p )
        ...
        if ( allocated(tmp1_field) .and. &
            ...
            deallocate( tmp1_field, fx_field, fy_field )
        end if
        ...
        do iter = 1, num_iter
            ...
            ! interface fluxes of tmp1: fx on (i-1/2, j), fy on (i, j-1/2),
            ! both needed one point beyond the interior
            if ( use_limiter ) then
                do k = 1, nz
                do j = j0, j1
                do i = i0, i1 + 1
                    fx_field(i, j, k) = tmp1_field(i, j, k) - tmp1_field(i - 1, j, k)
                    ! Xue (2000) Eq. 10; sign(0)=0, hence <=, which also
                    ! zeroes fluxes across flat interfaces
                    if ( -fx_field(i, j, k) * (in_field(i, j, k) - in_field(i - 1, j, k)) <= 0.0_wp ) then
                        ! count only real limiter action (nonzero flux zeroed)
                ...
                do j = j0, j1 + 1
                do i = i0, i1
                    fy_field(i, j, k) = tmp1_field(i, j, k) - tmp1_field(i, j - 1, k)
                    if ( -fy_field(i, j, k) * (in_field(i, j, k) - in_field(i, j - 1, k)) <= 0.0_wp ) then
            ...
            ! flux-divergence update (== in - alpha * lap(lap(in)) when unlimited)
            do k = 1, nz
            do j = j0, j1
            do i = i0, i1
                out_field(i, j, k) = in_field(i, j, k) - alpha * (      &
    ...
    subroutine read_cmd_line_arguments()
        ...
        call error(num_halo < 2 .or. num_halo > 8, &
                   "Please provide a reasonable value of num_halo (2..8; order-n filter needs n/2)")
