! Program: stencil2d-filter-kblock
...
!
! k-blocked variant of stencil2d-filter: temporaries are 2D per-k planes
! rather than full 3D arrays. --branchfree replaces the IF limiter with
! merge().
...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p )
        ...
        real (kind=wp), save, allocatable :: fx_plane(:, :), fy_plane(:, :)
        ...
        if ( allocated(p1_field) .and. &
            ...
            deallocate( p1_field, p2_field, fx_plane, fy_plane )
        end if
        if ( .not. allocated(p1_field) ) then
            allocate( p1_field(nx + 2 * num_halo, ny + 2 * num_halo) )
            allocate( p2_field(nx + 2 * num_halo, ny + 2 * num_halo) )
            allocate( fx_plane(nx + 2 * num_halo, ny + 2 * num_halo) )
            allocate( fy_plane(nx + 2 * num_halo, ny + 2 * num_halo) )
            p1_field = 0.0_wp
            p2_field = 0.0_wp
            fx_plane = 0.0_wp
            fy_plane = 0.0_wp
        ...
        do iter = 1, num_iter
            ...
            do k = 1, nz
                ...
                if ( use_limiter ) then
                    if ( branchfree ) then
                        ! merge-based limiter: no branch, no counter
                        do j = j0, j1
                        do i = i0, i1 + 1
                            fx_plane(i, j) = srcp(i, j) - srcp(i - 1, j)
                            fx_plane(i, j) = merge( 0.0_wp, fx_plane(i, j), &
                                sgn * fx_plane(i, j) * (in_field(i, j, k) - in_field(i - 1, j, k)) <= 0.0_wp )
                        end do
                        end do
                        do j = j0, j1 + 1
                        do i = i0, i1
                            fy_plane(i, j) = srcp(i, j) - srcp(i, j - 1)
                            fy_plane(i, j) = merge( 0.0_wp, fy_plane(i, j), &
                                sgn * fy_plane(i, j) * (in_field(i, j, k) - in_field(i, j - 1, k)) <= 0.0_wp )
                        end do
                        end do
                    else
                        do j = j0, j1
                        do i = i0, i1 + 1
                            fx_plane(i, j) = srcp(i, j) - srcp(i - 1, j)
                            if ( sgn * fx_plane(i, j) * (in_field(i, j, k) - in_field(i - 1, j, k)) <= 0.0_wp ) then
                                if ( fx_plane(i, j) /= 0.0_wp ) limited_count = limited_count + 1
                                fx_plane(i, j) = 0.0_wp
                            end if
                        end do
                        end do
                        ...
                        do i = i0, i1
                            fy_plane(i, j) = srcp(i, j) - srcp(i, j - 1)
                            if ( sgn * fy_plane(i, j) * (in_field(i, j, k) - in_field(i, j - 1, k)) <= 0.0_wp ) then
                                if ( fy_plane(i, j) /= 0.0_wp ) limited_count = limited_count + 1
                                fy_plane(i, j) = 0.0_wp
                            end if
                        end do
                        end do
                ...
                do i = i0, i1
                    out_field(i, j, k) = in_field(i, j, k) + alpha_eff * (  &
                          fx_plane(i + 1, j) - fx_plane(i, j)               &
                        + fy_plane(i, j + 1) - fy_plane(i, j) )
                end do
                end do
