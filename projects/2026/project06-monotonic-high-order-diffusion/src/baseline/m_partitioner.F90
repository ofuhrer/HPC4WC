...
contains
  ...
  function scatter_f32(this, field, root) result(r)
    ...
    if (this%rank_ == root_) then
      ...
      do rank = 0, this%num_ranks_ - 1
        ! domains_ bounds are 1-based inclusive; the course version added +1
        ! here, translating every slab by one cell (masked by halo healing
        ! and gather's inverse shift, but corrupting one row/column at the
        ! periodic wrap for boundary-nonzero fields)
        i_start = this%domains_(rank, 1)
        j_start = this%domains_(rank, 2)
