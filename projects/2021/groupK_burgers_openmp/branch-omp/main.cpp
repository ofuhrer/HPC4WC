...
try {
	...
	parser["threads"]
		.abbreviation('n')
		.description("set the number of threads (default: 1)")
		.bind(threads);
	parser["sideLength"]
		.abbreviation('l')
	...
	if(threads < 1)
		throw std::range_error("at least one thread required");
	...
	std::cout << threads << "-threaded benchmark:" << benchmark(task, streak);
