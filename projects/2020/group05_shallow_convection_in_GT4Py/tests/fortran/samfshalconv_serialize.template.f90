program samfshalconv_serialize
    real(kind=8),parameter:: grav   =9.80665e+0_8
    real(kind=8),parameter:: cp     =1.0046e+3_8
    real(kind=8),parameter:: hvap   =2.5000e+6_8
    real(kind=8),parameter:: rv     =4.6150e+2_8
    real(kind=8),parameter:: rd     =2.8705e+2_8
    real(kind=8),parameter:: fv     =rv/rd-1.
    real(kind=8),parameter:: t0c    =2.7315e+2_8
    real(kind=8),parameter:: cvap   =1.8460e+3_8
    real(kind=8),parameter:: cliq   =4.1855e+3_8
    real(kind=8),parameter:: eps    =rd/rv
    real(kind=8),parameter:: epsm1  =rd/rv-1.

    real(kind=8),parameter:: ttp    =2.7316e+2_8
    real(kind=8),parameter:: csol   =2.1060e+3_8
    real(kind=8),parameter:: hfus   =3.3358e+5_8
    real(kind=8),parameter:: psat   =6.1078e+2_8

    integer :: im, ix, km, itc, ntc, ntk, ntr, ncloud
    integer, dimension(:), allocatable :: islimsk, kcnv, kbot, ktop, kcnv_ref, kbot_ref, ktop_ref

    real (kind=8) :: clam, c0s, c1, asolfac, pgcon, delt
    real (kind=8), dimension(:), allocatable :: psp, garea, hpbl, fscav, rn, rn_ref
    real (kind=8), dimension(:, :), allocatable :: delp, prslp, dot, phil, &
        q1, t1, u1, v1, cnvw, cnvc, ud_mf, dt_mf, q1_ref, t1_ref, u1_ref, v1_ref, &
        cnvw_ref, cnvc_ref, ud_mf_ref, dt_mf_ref
    real (kind=8), dimension(:, :, :), allocatable :: qtr, qtr_ref
    !character(:), allocatable :: datapath
  ...
  elemental function fpvs(t)
    ...
    real(kind=8),parameter:: tliq=ttp
    real(kind=8),parameter:: tice=ttp-20.0
    real(kind=8),parameter:: dldtl=cvap-cliq
    real(kind=8),parameter:: heatl=hvap
    real(kind=8),parameter:: xponal=-dldtl/rv
    real(kind=8),parameter:: xponbl=-dldtl/rv+heatl/(rv*ttp)
    real(kind=8),parameter:: dldti=cvap-csol
    real(kind=8),parameter:: heati=hvap+hfus
    real(kind=8),parameter:: xponai=-dldti/rv
    real(kind=8),parameter:: xponbi=-dldti/rv+heati/(rv*ttp)
    real(kind=8) tr,w,pvl,pvi
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    tr=ttp/t
    if(t.ge.tliq) then
      fpvs=psat*(tr**xponal)*exp(xponbl*(1.-tr))
    elseif(t.lt.tice) then
      fpvs=psat*(tr**xponai)*exp(xponbi*(1.-tr))
    else
      w=(t-tice)/(tliq-tice)
...
!
      real(kind=8), intent(in) :: fscav(ntc)
...
!  local variables
      ...
      integer              kpbl(im)
!
      real(kind=8) clamd,   tkemx,   tkemn,   dtke
!
      real(kind=8) dellat,  delta, &
     &                     c0l,     d0, &
     &                     desdt,   dp, &
     &                     dq,      dqsdp,   dqsdt,   dt, &
...
!  local variables and arrays
      real(kind=8) pfld(im,km),    to(im,km),     qo(im,km), &
     &                     uo(im,km),      vo(im,km),     qeso(im,km), &
     &                     ctr(im,km,ntr), ctro(im,km,ntr)
!  for aerosol transport
      real(kind=8) qaero(im,km,ntc)
!  for updraft velocity calculation
      real(kind=8) wu2(im,km),     buo(im,km),    drag(im,km)
      real(kind=8) wc(im),         scaldfunc(im), sigmagfm(im)
!
!  cloud water
!     real(kind=8) qlko_ktcon(im), dellal(im,km), tvo(im,km),
      real(kind=8) qlko_ktcon(im), dellal(im,km), &
...
!
      real(kind=8) tf, tcr, tcrf
      parameter (tf=233.16, tcr=263.16, tcrf=1.0/(tcr-tf))

!------------------SERIALIZATION---------------------------------------
      !ser init directory='/data_out' prefix='Serialized' prefix_ref='SavedData'
!
!-----------------------------------------------------------------------
...
end program samfshalconv_serialize
