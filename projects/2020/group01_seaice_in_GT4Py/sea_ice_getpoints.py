...
#  --- ...  set flag for sea-ice
    ...
    if True:
        ...
        gridpoint = {}
        for var in IN_VARS:
            if np.shape(eval(var)) == ():
                gridpoint[var] = eval(var)
            elif np.shape(eval(var)) == (2304,):
                gridpoint[var] = eval(var)[i]
            else:
                gridpoint[var] = eval(var)[i,:]
        save_obj(gridpoint, 'land_point')
        assert False

    flag = (islimsk == 2) & flag_iter
    if any(flag):
        i = np.min(np.where(flag))
        gridpoint = {}
        for var in IN_VARS:
            if np.shape(eval(var)) == ():
                gridpoint[var] = eval(var)
                ...
                gridpoint[var] = eval(var)[i,:]
        save_obj(gridpoint, 'sea_ice_point')
        assert False
