...
contains
    ...
    !
    !   in_field          -- input field (nx x ny x nz with halo in the horizontal directions)
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        do iter = 1, num_iter
            ...
            !$acc data copyin( in_field ) create( tmp1_field, tmp2_field ) copyout( out_field )
            ...
            ! update in_field with out_field (except for last iteration)
        ...
        do j = 1, num_halo
        do i = 1 + num_halo, nx + num_halo
            out_field(i, j, k) = out_field(i, j + ny, k)
        end do
        end do
        end do
        ...
        do j = 1, ny + 2 * num_halo
        do i = 1, num_halo
            out_field(i, j, k) = out_field(i + nx, j, k)
        end do
        end do
        end do
