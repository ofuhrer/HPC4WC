! ******************************************************
!     Program: stencil2d-sendrecv
!     Author: Julia Dworzak
!     Email: dworzakj@ethz.ch
!     Date: 26.07.2022
!     Description: Parallel stencil example (4th-order diffusion) with sendrecv communication
! ******************************************************
...
program main
    ...
    integer :: timer_work, timer_halo
    real (kind=8) :: runtime, runtime_halo
    ...
    if ( is_master() ) then
        write(*, '(a)') '# ranks nx ny ny nz num_iter time time_halo'
    ...
    do cur_setup = 0, num_setups - 1
        ...
        ! warmup caches
        call apply_diffusion( f_in, f_out, alpha, num_iter=1, p=p, timer_halo=timer_halo )
        ...
        call apply_diffusion( f_in, f_out, alpha, num_iter=num_iter, p=p, timer_halo=timer_halo )
        ...
        if ( is_master() ) &
            write(*, '(a, i5, a, i5, a, i5, a, i5, a, i8, a, e15.7, a, e15.7, a)') &
                '[', num_rank(), ',', global_nx, ',', global_ny, ',', global_nz, &
                ',', num_iter, ',', runtime, ',', runtime_halo, '], \'
...
contains
    ...
    !
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p, timer_halo )
        implicit none
        ...
        integer, intent(inout) :: timer_halo
...
subroutine update_halo( field, p )
    ...
    ny = dims(2) - 2 * p%num_halo()
        ...
        ! send tb-buffers
        call MPI_Sendrecv(sndbuf_t, tb_size, dtype, p%top(), 1000, rcvbuf_b, tb_size, dtype, p%bottom(), 1000, p%comm(), status, ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_sendrecv(top)', code=ierror)
        call MPI_Sendrecv(sndbuf_b, tb_size, dtype, p%bottom(), 1001, rcvbuf_t, tb_size, dtype, p%top(), 1001, p%comm(), status, ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_sendrecv(bottom)', code=ierror)
        ...
        ! send lr-buffers
        call MPI_Sendrecv(sndbuf_r, lr_size, dtype, p%right(), 1002, rcvbuf_l, lr_size, dtype, p%left(), 1002 , p%comm(), status, ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_sendrecv(right)', code=ierror)
        call MPI_Sendrecv(sndbuf_l, lr_size, dtype, p%left(), 1003, rcvbuf_r, lr_size, dtype, p%right(), 1003, p%comm(), status, ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_sendrecv(left)', code=ierror)
