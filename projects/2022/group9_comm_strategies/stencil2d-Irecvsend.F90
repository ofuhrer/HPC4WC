! ******************************************************
!     Program: stencil2d-Irescvsend
!     Author: Victoria Bauer, Sarina Danioth, Julia Dworzak
!     Email: vbauer@ethz.ch
!     Date: 26.07.2022
!     Description: Parallel stencil example (4th-order diffusion) with Irecvsend communication
! ******************************************************
...
contains
    ...
    subroutine update_halo( field, p )
        ...
        ! send tb-buffers
        call MPI_Send(sndbuf_r, lr_size, dtype, p%right(), 1002, p%comm(), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Send(right)', code=ierror)
        call MPI_Send(sndbuf_l, lr_size, dtype, p%left(), 1003, p%comm(), ierror)
