! ******************************************************
!   Program: stencil2d-evenodd
!   Author: Victoria Bauer, Sarina Danioth & Julia Dworzak
!   Email: vbauer@ethz.ch
!   Date: 01.08.2022
!   Description: Parallel stencil example (4th-order diffusion) with evenodd communication
! ******************************************************
...
contains
    ...
    !
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p, timer_halo=timer_halo )
        implicit none
    ...
    subroutine update_halo( field, p )
        ...
        !get your own position on the partition grid
        pos = p%position()
        y_pos = pos(1)
        x_pos = pos(2)
        
        do evenodd = 0, 1
            if ( mod((mod(x_pos,2) + mod(y_pos,2)),2) == evenodd ) then
                !send tb
                call MPI_Send(sndbuf_t, tb_size, dtype, p%top(), 1000, p%comm(), ierror)
                call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Send(top)', code=ierror)
                call MPI_Send(sndbuf_b, tb_size, dtype, p%bottom(), 1001, p%comm(), ierror)
                call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Send(bottom)', code=ierror)
            else
                !receive bt
                call MPI_Recv(rcvbuf_b, tb_size, dtype, p%bottom(), 1000, p%comm(), status, ierror)
                call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Recv(bottom)', code=ierror)
                call MPI_Recv(rcvbuf_t, tb_size, dtype, p%top(), 1001, p%comm(), status, ierror)
        ...
        do evenodd = 0, 1
            if ( mod((mod(x_pos,2)+ mod(y_pos,2)), 2) == evenodd ) then
                !send lr
                call MPI_Send(sndbuf_l, lr_size, dtype, p%left(), 1002, p%comm(), ierror)
                call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Send(left)', code=ierror)
                call MPI_Send(sndbuf_r, lr_size, dtype, p%right(), 1003, p%comm(), ierror)
                call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Send(right)', code=ierror)
            else
                !receive rl
                call MPI_Recv(rcvbuf_r, lr_size, dtype, p%right(), 1002, p%comm(), status, ierror)
                call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Recv(right)', code=ierror)
                call MPI_Recv(rcvbuf_l, lr_size, dtype, p%left(), 1003, p%comm(), status, ierror)
