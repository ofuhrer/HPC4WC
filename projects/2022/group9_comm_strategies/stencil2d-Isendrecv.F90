! ******************************************************
!     Program: stencil2d-Isendrecv
!     Author: Victoria Bauer, Sarina Danioth, Julia Dworzak
!     Email: dworzakj@ethz.ch
!     Date: 20.07.2022
!     Description: Parallel stencil example (4th-order diffusion) with  Isendrecv communication
! ******************************************************
...
contains
    ...
    subroutine update_halo( field, p )
        ...
        ! send tb-buffers
        call MPI_Isend(sndbuf_b, tb_size, dtype, p%bottom(), 1000, p%comm(), tb_req(1), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(bottom)', code=ierror)
        call MPI_Isend(sndbuf_t, tb_size, dtype, p%top(), 1001, p%comm(),tb_req(2), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(top)', code=ierror)
        
        ! receive tb-buffers
        call MPI_Recv(rcvbuf_t, tb_size, dtype, p%top(), 1000, p%comm(), status, ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Recv(top)', code=ierror)
        call MPI_Recv(rcvbuf_b, tb_size, dtype, p%bottom(), 1001, p%comm(), status, ierror)
        ...
        ! send lr-buffers
        call MPI_Isend(sndbuf_l, lr_size, dtype, p%left(), 1002, p%comm(), lr_req(1), ierror)
        call error(ierror /= MPI_SUCCESS, 'Problem with MPI_Isend(left)', code=ierror)
        call MPI_Isend(sndbuf_r, lr_size, dtype, p%right(), 1003, p%comm(), lr_req(2), ierror)
