...
program main
    ...
    do cur_setup = 0, num_setups - 1
        ...
        if ( .not. scan .and. is_master() ) &
            call write_field_to_file( in_field, num_halo, "in_field_orig.dat" )

        ! warmup caches
        call apply_diffusion( in_field, out_field, alpha, num_iter=1, timer_halo=timer_halo )
        ...
        call apply_diffusion( in_field, out_field, alpha, num_iter=num_iter, timer_halo=timer_halo )
        ...
        if ( .not. scan .and. is_master() ) &
            call write_field_to_file( out_field, num_halo, "out_field_orig.dat" )
...
contains
    ...
    !
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, timer_halo )
        implicit none
