...
program main
    ...
    use m_utils, only: timer_init, timer_reset, timer_start, timer_end, timer_get, is_master, &
        num_rank, write_field_to_file
    ...
    ! local
    integer :: global_nx, global_ny, global_nz, num_iter, num_halo
    integer :: num_zhalo = 2
    logical :: scan

    real (kind=wp) :: alpha = 1.0_wp / 100.0_wp

    real (kind=wp), allocatable :: in_field(:, :, :), f_in(:, :, :)
    real (kind=wp), allocatable :: out_field(:, :, :), f_out(:, :, :)

    integer :: timer_work = -999, timer_halo = -999, timer_lap = -999
    real (kind=8) :: runtime, runtime_halo, runtime_lap
    ...
    if ( is_master() ) then
        write(*, '(a)') '# ranks nx ny nz num_iter time time_halo'
    ...
    do cur_setup = 0, num_setups - 1
        ...
        p = Partitioner(MPI_COMM_WORLD, (/global_nx, global_ny, global_nz + 2*num_zhalo/), num_halo, periodic=(/.true., .true./))

        f_in = p%scatter(in_field, root=0)
        allocate(f_out, source=f_in)

        ! warmup caches
        call apply_diffusion( f_in, f_out, alpha, num_iter=1, p=p, timer_halo=timer_halo, timer_lap=timer_lap, &
            num_zhalo=num_zhalo)
        ...
        call apply_diffusion( f_in, f_out, alpha, num_iter=num_iter, p=p, timer_halo=timer_halo, timer_lap=timer_lap, &
            num_zhalo=num_zhalo)
        ...
        call update_halo( f_out, p , num_zhalo)
        ...
        runtime_halo = timer_get( timer_halo )
        runtime_lap = timer_get( timer_lap )
        if ( is_master() ) &
            write(*, '(a, i5, a, i5, a, i5, a, i5, a, i8, a, e15.7, a, e15.7, a, e15.7, a)') &
            '[', num_rank(), ',', global_nx, ',', global_ny, ',', global_nz, &
            ',', num_iter, ',', runtime, ',', runtime_halo, ',', runtime_lap, '], \'
...
contains
    ...
    !
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter, p, timer_halo, timer_lap, num_zhalo)
        implicit none
        ...
        integer, intent(in) :: num_iter, num_zhalo
        type(Partitioner), intent(in) :: p
        integer, intent(inout) :: timer_halo, timer_lap
        ...
        if ( allocated(tmp1_field) .and. &
            any( shape(tmp1_field) /= (/nx + 2 * num_halo, ny + 2 * num_halo, nz + 2*num_zhalo/) ) ) then
            deallocate( tmp1_field, tmp2_field )
        end if
        if ( .not. allocated(tmp1_field) ) then
            allocate( tmp1_field(nx + 2 * num_halo, ny + 2 * num_halo, nz + 2*num_zhalo) )
            allocate( tmp2_field(nx + 2 * num_halo, ny + 2 * num_halo, nz + 2*num_zhalo ))
            tmp1_field = 0.0_wp
            tmp2_field = 0.0_wp
        end if
        ...
        do iter = 1, num_iter
            ...
            call timer_start('halo', timer_halo)

            call update_halo( in_field, p , num_zhalo=num_zhalo)

            call timer_end( timer_halo ) 
            
            call timer_start('lap', timer_lap)

            call laplacian( in_field, tmp1_field, num_halo, num_zhalo, extend=1 )
            call laplacian( tmp1_field, tmp2_field, num_halo, num_zhalo, extend=0 )
            
            call timer_end(timer_lap)

            ! do forward in time step
            do k = lbound(out_field,3) + num_zhalo, ubound(out_field,3) - num_zhalo
            ...
            if ( iter /= num_iter ) then
                do k = lbound(in_field,3) + num_zhalo , ubound(in_field,3) - num_zhalo
    ...
    !
    subroutine laplacian( field, lap, num_halo, num_zhalo, extend )
        implicit none

        ! argument
        real (kind=wp), intent(in) :: field(:, :, :)
        real (kind=wp), intent(inout) :: lap(:, :, :)
        integer, intent(in) :: num_halo, num_zhalo, extend

        ! local
        integer :: i, j, k

        do k = lbound(field,3) + num_zhalo - extend, ubound(field,3) - num_zhalo + extend
        ...
        do i = lbound(field,1) + num_halo - extend, ubound(field,1) - num_halo + extend
            lap(i, j, k) = -6._wp * field(i, j, k)      & ! changed 6 to 4
    ...
    !
    subroutine update_halo( field, p, num_zhalo )
        ...
        ! halo update in z-direction
        do k = 1, num_zhalo
        do j = 1, ny + 2*num_halo
        do i = 1, nx + 2*num_halo
            field(i,j,k) = field(i, j, k + nz)
        end do
        end do
        end do
         
         
        do k = nz + num_zhalo + 1, nz + 2 * num_zhalo
        do j = 1, ny + 2*num_halo
        do i = 1, nx + 2*num_halo
            field(i, j, k) = field(i,j,k - nz)
        end do
    ...
    subroutine setup()
        ...
        allocate( in_field(global_nx + 2 * num_halo, global_ny + 2 * num_halo, global_nz + 2 * num_zhalo) )
        in_field = 0.0_wp
        do k = 1 + num_zhalo + global_nz / 4, num_zhalo + 3 * global_nz / 4 
        ...
        allocate( out_field(global_nx + 2 * num_halo, global_ny + 2 * num_halo, global_nz + 2 * num_zhalo) )
        out_field = in_field
    ...
    subroutine read_cmd_line_arguments()
        ...
        call error(num_halo < 2 .or. num_halo > 100, "Please provide a reasonable value of num_halo")
