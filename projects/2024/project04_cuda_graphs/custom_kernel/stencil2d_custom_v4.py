...
# 2D grid, 2D block
# shared memory, inner elements read from shared memory
...
void _laplacian(const double* in_field, double* lap_field, int nz, int ny, int nx, int padding) {
    ...
    for(int idz = tidz; idz < nz; idz += gridDim.z) {
        ...
        if(tidx < nx_domain && tidy < ny_domain) {
            double tmp;
            tmp = -4.0 * shm[(threadIdx.y + 1) * (blockDim.x + 2) + threadIdx.x + 1];

            if(threadIdx.x == 0) {
                tmp += in_field[idz * nx * ny + idy*nx + idx - 1];
            }
            else{
                tmp += shm[(threadIdx.y + 1)*(blockDim.x+2) + threadIdx.x + 1 - 1];
            }
            ...
            if(threadIdx.y == 0) {
                tmp += in_field[idz * nx * ny + (idy-1)*nx + idx];
            }
            else{
                tmp += shm[(threadIdx.y + 1 - 1)*(blockDim.x+2) + threadIdx.x + 1];
            }
            ...
            lap_field[idz*nx*ny + idy*nx + idx] = tmp;
        }
