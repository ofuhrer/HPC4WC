...
def apply_diffusion(in_field, out_field, alpha, num_halo, num_iter=1):
    ...
    # Capture a CUDA graph for a single diffusion iteration.
    with stream:
        ...
        # Here we need to use memory copy operations to swap the fields
        # because we cannot swap the fields directly in a simple, single
        # CUDA graph.
