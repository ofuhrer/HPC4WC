...
def apply_diffusion(in_field, out_field, alpha, num_halo, num_iter=1):
    ...
    # Capture CUDA graph for two iterations of the diffusion equation.
    with stream:
        ...
        # Since we swap twice, we are back to the original in_field and
        # out_field and our graph is complete.
        in_field, out_field = out_field, in_field

        # End capturing the operations and create a CUDA graph.
        graph = stream.end_capture()

    # Launch the graph for half the number of iterations except the last one.
    for __ in range((num_iter - 1) // 2):
        graph.launch(stream)
...
def main(
    ...
    assert num_iter % 2 == 1, "num_iter must be odd for this implementation!"
    assert (
        2 <= num_halo <= 256
