...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        do iter = 1, num_iter
            ...
            do k = 1, nz
                call diffusion_2d( in_field(:,:,k), tmp1_field(:,:,k), num_halo, extend=1 )
                
            do j = 1 + num_halo, ny + num_halo
            do i = 1 + num_halo, nx + num_halo
                out_field(i, j, k) = tmp1_field(i, j, k)
            end do
            end do
    ...
    subroutine diffusion_2d( field, dif, num_halo, extend )
        implicit none
            
        ! argument
        real (kind=wp), intent(in) :: field(:, :)
        real (kind=wp), intent(inout) :: dif(:, :)
        ...
        do i = 1 + num_halo - extend, nx + num_halo + extend
            dif(i, j) = 0.25 * (field(i - 1, j) + field(i + 1, j)  + field(i, j - 1) + field(i, j + 1))
        end do
        end do
