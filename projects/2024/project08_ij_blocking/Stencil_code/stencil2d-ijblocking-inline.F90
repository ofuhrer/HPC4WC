...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        if (allocated(tmp1) .and. &
            any(shape(tmp1) /= (/nx + 2 * num_halo, ny + 2 * num_halo/))) then
            deallocate(tmp1)
        end if
        if (.not. allocated(tmp1)) then
            allocate(tmp1(nx + 2 * num_halo, ny + 2 * num_halo))
            tmp1 = 0.0_wp
            tmp2 = 0.0_wp
        end if
