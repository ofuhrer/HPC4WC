...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        real (kind=wp) :: tmp2
        integer :: iter, i, j, k, j_end, i_end, size_i, size_j
        ...
        do iter = 1, num_iter
            ...
            do j = 1 + num_halo, size_j + num_halo
            do i = 1 + num_halo, size_i + num_halo
                if (j==1 .and. i == 1) then
                    tmp1(i, j - 1) = -4._wp * in_field(i + i_end - size_i, j - 1 + j_end - size_j, k)      &
                        + in_field(i - 1 + i_end - size_i, j - 1 + j_end - size_j, k) &
                        + in_field(i + 1 + i_end - size_i, j - 1 + j_end - size_j, k)  &
                        + in_field(i + i_end - size_i, j - 2 + j_end - size_j, k) &
                        + in_field(i + i_end - size_i, j + j_end - size_j, k)
                    tmp1(i - 1, j) = -4._wp * in_field(i - 1 + i_end - size_i, j + j_end - size_j, k)      &
                        + in_field(i - 2 + i_end - size_i, j + j_end - size_j, k) &
                        + in_field(i + i_end - size_i, j + j_end - size_j, k)  &
                        + in_field(i - 1 + i_end - size_i, j - 1 + j_end - size_j, k) &
                        + in_field(i - 1 + i_end - size_i, j + 1 + j_end - size_j, k)
                    tmp1(i, j) = -4._wp * in_field(i + i_end - size_i, j + j_end - size_j, k)      &
                        + in_field(i - 1 + i_end - size_i, j + j_end - size_j, k) &
                        + in_field(i + 1 + i_end - size_i, j + j_end - size_j, k)  &
                        + in_field(i + i_end - size_i, j - 1 + j_end - size_j, k) &
                        + in_field(i + i_end - size_i, j + 1 + j_end - size_j, k)
                    tmp1(i + 1, j) = -4._wp * in_field(i + 1 + i_end - size_i, j + j_end - size_j, k)      &
                        + in_field(i + i_end - size_i, j + j_end - size_j, k) &
                        + in_field(i + 2 + i_end - size_i, j + j_end - size_j, k)  &
                ...
                tmp2 = -4._wp * tmp1(i, j)      &
                    + tmp1(i - 1, j) + tmp1(i + 1, j)  &
                    + tmp1(i, j - 1) + tmp1(i, j + 1)
                
                out_field(i + i_end - size_i, j + j_end - size_j, k) = in_field(i + i_end - size_i, j + j_end - size_j, k) &
                - alpha * tmp2
            end do
            end do
            end do
