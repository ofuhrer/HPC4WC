...
contains
        ...
        do iter = 1, num_iter
            ...
            do j = 1 + num_halo, ny + num_halo
                if (j==1+num_halo) then
                    call laplacian_1d(in_field(:,j-2:j,k), tmp1_field(:,j-1,k), num_halo, extend=1 )
                    call laplacian_1d(in_field(:,j-1:j+1,k), tmp1_field(:,j,k), num_halo, extend=1 )
                end if
                call laplacian_1d( in_field(:,j:j+2,k), tmp1_field(:,j+1,k), num_halo, extend=1 )
                call laplacian_1d( tmp1_field(:,j-1:j+1,k), tmp2_field(:,j,k), num_halo, extend=0 )
    ...
    !
    subroutine laplacian_1d( field, lap, num_halo, extend )
        implicit none
        ...
        do i = 1 + num_halo - extend, nx + num_halo + extend
            lap(i) = -4._wp * field(i, 2)      &
                + field(i - 1, 2) + field(i + 1, 2)  &
                + field(i, 1) + field(i, 3)
        end do
