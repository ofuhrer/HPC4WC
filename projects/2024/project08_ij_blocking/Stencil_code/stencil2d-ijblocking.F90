    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        do iter = 1, num_iter
            ...
            do i = 1 + num_halo, nx + num_halo
                if (j==1+num_halo .and. i == 1+num_halo) then
                    call laplacian(in_field(i-2:i,j-2:j,k), tmp1_field(i-1,j-1,k), num_halo, extend=1 )
                    call laplacian(in_field(i-1:i+1,j-2:j,k), tmp1_field(i,j-1,k), num_halo, extend=1 )
                    call laplacian(in_field(i-2:i,j-1:j+1,k), tmp1_field(i-1,j,k), num_halo, extend=1 )
                    call laplacian(in_field(i-1:i+1,j-1:j+1,k), tmp1_field(i,j,k), num_halo, extend=1 )
                end if
                if (j==1+num_halo) then
                    call laplacian(in_field(i-1:i+1,j-2:j,k), tmp1_field(i+1,j-1,k), num_halo, extend=1 )
                    call laplacian(in_field(i-1:i+1,j-1:j+1,k), tmp1_field(i+1,j,k), num_halo, extend=1 )
                end if 
                if (i==1+num_halo) then
                    call laplacian(in_field(i-2:i,j:j+2,k), tmp1_field(i-1,j+1,k), num_halo, extend=1 )
                    call laplacian(in_field(i-1:i+1,j:j+2,k), tmp1_field(i,j+1,k), num_halo, extend=1 )
                end if
                
                call laplacian( in_field(i:i+2,j:j+2,k), tmp1_field(i+1,j+1,k), num_halo, extend=1 )
                call laplacian( tmp1_field(i-1:i+1,j-1:j+1,k), tmp2_field(i,j,k), num_halo, extend=0 )
                ...
                if ( iter /= num_iter ) then
                    do j = 1 + num_halo, ny + num_halo
                    do i = 1 + num_halo, nx + num_halo
                        in_field(i, j, k) = out_field(ih, j, k)
                    end do
                    end do
                end if
