# 1 "stencil2d-simplecopy-block.F90"
! ******************************************************
!     Program: stencil2d
...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        if ( allocated(tmp1_field) .and. &
            any( shape(tmp1_field) /= (/num_halo + size_i, num_halo + size_j /) ) ) then
            deallocate( tmp1_field)
        end if
        if ( .not. allocated(tmp1_field) ) then
            allocate( tmp1_field(num_halo + size_i, num_halo + size_j) )
            tmp1_field = 0.0_wp
        end if
        ...
        do iter = 1, num_iter
            ...
            do i = 1 + num_halo, size_i + num_halo
                tmp1_field(i, j) = in_field(i + i_end - size_i, j + j_end - size_j, k)
                out_field(i + i_end - size_i, j + j_end - size_j, k) = tmp1_field(i, j)
            end do
            end do
            end do
            end do
