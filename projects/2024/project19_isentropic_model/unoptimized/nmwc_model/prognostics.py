...
def prog_isendens(sold, snow, unow, dtdx, dthetadt=None):
    ...
    if idthdt == 1:
        ii,kk = np.ix_(i,k) 
        snew[ii,kk] = snew[ii,kk] - dt/dth*(snow[ii,kk+1]*(dthetadt[ii,kk+2] + dthetadt[ii,kk+1])/2 - snow[ii, kk-1]*(dthetadt[ii,kk] + dthetadt[ii,kk-1])/2)
    # *** Exercise 2.1/5.2 isentropic mass density ***
...
def prog_moisture(unow, qvold, qcold, qrold, qvnow, qcnow, qrnow, dtdx, dthetadt=None):
    ...
    qcnew[i,:] = qcold[i,:] - (unow[i+1,:] + unow[i,:])/2*dtdx*(qcnow[i+1,:]-qcnow[i-1,:])

    qrnew[i,:] = qrold[i,:] - (unow[i+1,:] + unow[i,:])/2*dtdx*(qrnow[i+1,:]-qrnow[i-1,:])

    if idthdt == 1:
        ii,kk = np.ix_(i,k) 
        qvnew[ii,kk] = qvnew[ii,kk]-dt/dth*(qvnow[ii,kk+1] - qvnow[ii, kk-1]) * (dthetadt[ii,kk+1] + dthetadt[ii,kk])/2 
        qcnew[ii,kk] = qcnew[ii,kk]-dt/dth*(qcnow[ii,kk+1] - qcnow[ii, kk-1]) * (dthetadt[ii,kk+1] + dthetadt[ii,kk])/2 
        qrnew[ii,kk] = qrnew[ii,kk]-dt/dth*(qrnow[ii,kk+1] - qrnow[ii, kk-1]) * (dthetadt[ii,kk+1] + dthetadt[ii,kk])/2 
    #
    #
    # *** Exercise 4.1/5.2  ***
...
def prog_numdens(unow, ncold, nrold, ncnow, nrnow, dtdx, dthetadt=None):
    ...
    if idthdt == 1:
        ii,kk = np.ix_(i,k) 
        ncnew[ii,kk] = ncnew[ii,kk]-dt/dth*(ncnow[ii,kk+1] - ncnow[ii, kk-1]) * (dthetadt[ii,kk+1] + dthetadt[ii,kk])/2 
        nrnew[ii,kk] = nrnew[ii,kk]-dt/dth*(nrnow[ii,kk+1] - nrnow[ii, kk-1]) * (dthetadt[ii,kk+1] + dthetadt[ii,kk])/2 
