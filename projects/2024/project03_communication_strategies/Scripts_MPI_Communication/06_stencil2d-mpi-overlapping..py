...
# ******************************************************
# update_halo function and apply diffusion function modified by Group 3 for the
# HPCWC Project MPI Communication FS2024 ETHZ
#
# Students: Angelika Koch, Andri Heeb, Tim Zimmermann
# Date: 29.08.2024
# Description: Implement Overlapping communication
...
def update_halo(field,tmp_field, num_halo, p=None):
    ...
    #Here we could calculte the inner Laplacian field, since this shouldn't need anythin
    ###
    #laplacian(field, tmp_field, num_halo, extend=0)
    ###

    # wait and unpack this was an attempt thet dint work
    #for req in reqs_tb:
    #    req.barrier()

    #Advice from simon, but still din't work
    MPI.Request.Waitall(reqs_tb)
    ...
    ###
    #Calculate laplacian inner field
    laplacian(field, tmp_field, num_halo, extend=1)
    ###

    # wait and unpack
    #for req in reqs_tb:
    #    req.barrier()
    MPI.Request.Waitall(reqs_lr)
...
def apply_diffusion(in_field, out_field, alpha, num_halo, num_iter=1, p=None):
    ...
    for n in range(num_iter):
        ...
        update_halo(in_field, tmp_field, num_halo, p)

        #Comment out inner laplacian, since that is done overlapping
        #in the update_halo() function
        ...
        if n < num_iter - 1:
            ...
            update_halo(out_field, tmp_field, num_halo, p)
...
def main(nx, ny, nz, num_iter, num_halo=2, plot_result=False):
    ...
    update_halo(out_field, in_field, num_halo, p)
