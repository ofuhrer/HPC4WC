...
# Date: 29.08.2024
# Description: Changed MPI communication to Sendrecive
###
...
def update_halo(field, num_halo, p=None):
    ...
    #Sendrecv for top and bottom
    reqs_tb.append(p.comm().Sendrecv(sendbuf=b_sndbuf, dest=p.top(),recvbuf=b_rcvbuf, source=p.bottom()))
    reqs_tb.append(p.comm().Sendrecv(sendbuf=t_sndbuf, dest=p.bottom(),recvbuf=t_rcvbuf, source=p.top()))
    ...
    #allocate list for requests left and right
    reqs_lr = []
    ...
    #Sendrecv right and left
    reqs_lr.append(p.comm().Sendrecv(sendbuf=l_sndbuf, dest=p.right(),recvbuf=l_rcvbuf, source=p.left()))
    reqs_lr.append(p.comm().Sendrecv(sendbuf=r_sndbuf, dest=p.left(),recvbuf=r_rcvbuf, source=p.right()))
