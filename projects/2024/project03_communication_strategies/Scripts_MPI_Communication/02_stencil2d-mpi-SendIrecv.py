...
# Date: 29.08.2024
# Description: Changed MPI communication to Irecv and Send
###
...
def update_halo(field, num_halo, p=None):
    ...
    #bottom buffer
    b_rcvbuf = np.empty_like(field[:, 0:num_halo, num_halo:-num_halo]) #1:-1 means only the mid part
    #top buffer
    ...
    #add Irecv bottom
    reqs_tb.append(p.comm().Irecv(b_rcvbuf, source = p.bottom(),tag=1))
    #pack send
    b_sndbuf = field[:, -2 * num_halo:-num_halo, num_halo:-num_halo].copy()
    #add Send from bottom to top
    reqs_tb.append(p.comm().Send(b_sndbuf, dest = p.top(),tag=1))

    #add Irecv bottom
    reqs_tb.append(p.comm().Irecv(t_rcvbuf, source = p.top(),tag=2))
    #pack send
    t_sndbuf = field[:, num_halo:2 * num_halo, num_halo:-num_halo].copy()
    #add Send from top to bottom
    reqs_tb.append(p.comm().Send(t_sndbuf, dest = p.bottom(),tag=2))
    ...
    #left to right
    reqs_lr.append(p.comm().Irecv(l_rcvbuf, source = p.left(), tag=3))
    l_sndbuf = field[:, :, -2 * num_halo:-num_halo].copy()
    reqs_lr.append(p.comm().Send(l_sndbuf, dest = p.right(),tag=3))

    #right to left
    reqs_lr.append(p.comm().Irecv(r_rcvbuf, source = p.right(),tag=4))
    r_sndbuf = field[:, :, num_halo:2 * num_halo].copy()
    reqs_lr.append(p.comm().Send(r_sndbuf, dest = p.left(),tag=4))
