! ******************************************************
!     Program: stencil2d-sequential_nPoint,4
...
contains
    ...
    subroutine find_neighbors(neighbors, gridindex)
        ...
        ! (nPoint, 4); nPoint rows and 4 columns for neighbors (top, btm, left, right)
    ...
    subroutine apply_diffusion(in_field, out_field, alpha, num_iter)
        ...
        if (allocated(tmp1_field) .and. any(shape(tmp1_field) /= (/nPoint/))) then
