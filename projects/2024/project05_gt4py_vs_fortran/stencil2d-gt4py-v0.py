...
@gtscript.function
def laplacian(field):
    return - 4 * field[0,0,0] + field[-1,0,0] + field[1,0,0] + field[0,-1,0] + field[0,1,0]
    #pass
...
def diffusion_defs(
    ...
    with computation(PARALLEL), interval(...):
        #out_field = laplacian(in_field)
        #out_field[0,0,0] = in_field[0,0,0] - alpha * out_field
        
        tmp = laplacian(in_field)
        out_field[0,0,0] = in_field[0,0,0] - alpha * tmp[0,0,0]
    #pass
...
def apply_diffusion(
    ...
    shape = in_field.shape
    origin = (num_halo,num_halo,0)  # TODO
    domain = (shape[0] - 2 * num_halo, shape[1] - 2 * num_halo, shape[2])  # TODO
...
def main(nx, ny, nz, num_iter, num_halo=2, backend="numpy", plot_result=False):
    ...
    in_field = gt.storage.empty(backend, dorigin, (nx,ny,nz), dtype=float) #None  # TODO
    out_field = gt.storage.empty(backend, dorigin, (nx,ny,nz), dtype=float) #None  # TODO
