! ******************************************************
!     Program: stencil2d
!      Author: Oliver Fuhrer + Floriane Gabriel
!       Email: oliverf@vulcan.com
!        Date: 20.05.2020 + 19.08.2024
! Description: Simple stencil example (4th-order diffusion) + openACC k parallelised
! ******************************************************
...
program main
    ...
    integer :: nx_setups(6) = (/ 16,64,256,1024,4096,16384 /)
    integer :: ny_setups(1) = (/ 16,64,256,1024,4096,16384 /)
    ...
    if ( is_master() ) then
        write(*, '(a)') 'pd.DataFrame(columns=[''x'',''y'',''z'',''iter'',''time''],data=[\'
    end if
    ...
    do cur_setup = 0, num_setups - 1
        ...
        if ( is_master() ) &
            write(*, '(a, i5, a, i5, a, i5, a, i8, a, f10.3, a)') &
                '[', nx, ',', ny, ',', nz, ',', num_iter, ',', runtime, '], \'
...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        !$acc data copyin(in_field) create(tmp1_field) copyout(out_field)
        ...
        do iter = 1, num_iter
            ...
            !$acc loop gang
            do k = 1, nz
                do j = 0 + num_halo, ny + num_halo + 1
                do i = 0 + num_halo, nx + num_halo + 1
