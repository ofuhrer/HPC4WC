...
def main(nx, ny, nz, num_iter, num_halo=2, backend="numpy", plot_result=False):
    ...
    if p.rank() == 0:
      np.save("in_field-gt4py-mpi-laplacian", field_in)
      ...
      if plot_result:
          ...
          plt.colorbar()
          plt.savefig("in_field-gt4py-mpi-laplacian.png")
          plt.close()
    ...
    if p.rank() == 0:
      np.save("out_field-gt4py-mpi-laplacian", field_out)
      ...
      if plot_result:
          ...
          plt.colorbar()
          plt.savefig("out_field-gt4py-mpi-laplacian.png")
          plt.close()
