...
/**
 * @file cnn_opt.cpp
 * @brief Top level CNN forward using optimized sublayers.
 *
 * Sublayers implement their own internal parallelism. This file adds safe
 * batch parallelism for flatten and the final write back, plus a SIMD hint
 * on the innermost width loop during flatten. No algorithmic changes.
 */
...
CNNOpt::CNNOpt(int in_channels, int num_classes)
    : conv1_(in_channels, 8, 3, 1, 1),   // 1→8, 3x3, s=1, p=1
      relu1_(),
      pool1_(2, 2),                      // 2x2, s=2
      conv2_(8, 16, 3, 1, 1),            // 8→16, 3x3, s=1, p=1
      relu2_(),
      pool2_(2, 2),                      // 2x2, s=2
      fc_(16 * 7 * 7, num_classes)       // 784 → num_classes
{}

Tensor4D CNNOpt::forward(const Tensor4D& batch) const {
  // Each layer handles its own batch parallelism
  ...
  // Parallelize across samples; SIMD across innermost width dimension.
#ifdef _OPENMP
...
#pragma omp simd
        ...
        for (int j = 0; j < W; ++j) {
          flattened[static_cast<std::size_t>(n)]
                   [static_cast<std::size_t>(idx++)] = x_nci[static_cast<std::size_t>(j)];
        }
      }
    }
  }

  // Fully connected and reshape back to [N][1][1][num_classes]
