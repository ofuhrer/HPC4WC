...
// -------- small helpers kept local to avoid touching other files --------
...
struct ScopedComputeTimer {
    ...
    Clock::time_point t0;
    int samples;
    double* out_sec; // optional pointer to store value
    ...
    ~ScopedComputeTimer() {
        ...
        // Machine-readable single line preferred by the Python drivers
...
int main() {
    ...
    // Clamp N to available data if needed
    ...
    // Run model and measure compute time only
    CNNNaive cnn;
...

    {
        ...
        // Light postprocess: argmax per sample and accuracy
        for (int n = 0; n < N_eff; ++n) {
            int predicted = 0;
    ...
    // Summary line for accuracy that the drivers can parse if desired
    ...
    // Optional human-friendly one-liners when not quiet
    if (!QUIET) {
