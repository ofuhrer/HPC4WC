...
program main
    ...
    integer, allocatable :: random_lookup(:,:)
    integer, allocatable :: inv_random_lookup(:,:)
...
contains
    ...
    subroutine setup()
        ...
        call random_permutation(perm, nx*ny)

        ! initialize lookup tables
        if (allocated(random_lookup)) deallocate(random_lookup)
        if (allocated(inv_random_lookup)) deallocate(inv_random_lookup)
        allocate(random_lookup(nx, ny))
        allocate(inv_random_lookup(nx*ny, 2))
        n = 0
        do j = 1, ny
            do i = 1, nx
                n = n + 1
                random_lookup(i, j) = perm(n)
                inv_random_lookup(perm(n), 1) = i
                inv_random_lookup(perm(n), 2) = j
            end do
        end do
        do k = 1 + nz / 4, 3 * nz / 4
            do j = 1 + ny / 4, 3 * ny / 4
        ...
        do n = 1, nx*ny
            i = inv_random_lookup(n, 1)
            j = inv_random_lookup(n, 2)
            neighbor_lookup(n, 1) = random_lookup(modulo(i-2, nx)+1, j) ! left
            neighbor_lookup(n, 2) = random_lookup(modulo(i, nx)+1, j)   ! right
            neighbor_lookup(n, 3) = random_lookup(i, modulo(j-2, ny)+1) ! down
            neighbor_lookup(n, 4) = random_lookup(i, modulo(j, ny)+1)   ! up
        end do
    end subroutine setup


    ! helper function to generate a random permutation of integers 1..n
    ...
    ! function looks up n based on i, j, and nx
    ...
    ! function looks up i based on n and nx
    ...
    ! function looks up j based on n and nx
