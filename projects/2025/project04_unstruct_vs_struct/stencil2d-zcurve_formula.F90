...
contains
    ...
    subroutine apply_diffusion( in_field, out_field, alpha, num_iter )
        ...
        do iter = 1, num_iter
            ...
            do k = 1, nz
                do n = 1, nx*ny
                    nL = neighbor_n(n, 1, nx, ny)
                    nR = neighbor_n(n, 2, nx, ny)
                    nD = neighbor_n(n, 3, nx, ny)
                    nU = neighbor_n(n, 4, nx, ny)
    ...
    ! decoding morton encoded integer z into integer x
    elemental pure function morton_decode_x(z) result(x)
        integer, intent(in) :: z
        integer :: x, i
        x = 0
        do i = 0, 15
            x = setbit(x, i, ibits(z, 2*i, 1))
        end do
    end function morton_decode_x

    ! decoding morton encoded integer z into integer y
    elemental pure function morton_decode_y(z) result(y)
        integer, intent(in) :: z
        integer :: y, i
        y = 0
        do i = 0, 15
            y = setbit(y, i, ibits(z, 2*i+1, 1))
        end do
    end function morton_decode_y
    ...
    ! help function to compute coordinate i
    elemental pure function n_to_i(n, nx) result(i)
        integer, intent(in) :: n, nx
        integer :: i
        i = morton_decode_x(n-1) + 1
    end function n_to_i
    
    ! help function to compute coordinate j
    elemental pure function n_to_j(n, nx) result(j)
        integer, intent(in) :: n, nx
        integer :: j
        j = morton_decode_y(n-1) + 1
    end function n_to_j
