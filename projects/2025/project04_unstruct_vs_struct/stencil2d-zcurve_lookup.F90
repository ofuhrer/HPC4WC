...
program main
    ...
    ! Allocatable lookup tables for Morton and neighbor indices
    integer, allocatable :: morton_lookup(:,:)
    integer, allocatable :: inv_morton_lookup(:,:)
...
contains
    ...
    subroutine setup()
        ...
        ! initialize morton lookup tables
        if (allocated(morton_lookup)) deallocate(morton_lookup)
        if (allocated(inv_morton_lookup)) deallocate(inv_morton_lookup)
        allocate(morton_lookup(nx, ny))
        allocate(inv_morton_lookup(nx*ny, 2))
        n = 0
        do j = 1, ny
            do i = 1, nx
                n = morton_encode(i-1, j-1) + 1
                morton_lookup(i, j) = n
                inv_morton_lookup(n, 1) = i
                inv_morton_lookup(n, 2) = j
            end do
        end do
        do k = 1 + nz / 4, 3 * nz / 4
            do j = 1 + ny / 4, 3 * ny / 4
        ...
        do n = 1, nx*ny
            i = inv_morton_lookup(n, 1)
            j = inv_morton_lookup(n, 2)
            neighbor_lookup(n, 1) = morton_lookup(modulo(i-2, nx)+1, j) ! left
            neighbor_lookup(n, 2) = morton_lookup(modulo(i, nx)+1, j)   ! right
            neighbor_lookup(n, 3) = morton_lookup(i, modulo(j-2, ny)+1) ! down
            neighbor_lookup(n, 4) = morton_lookup(i, modulo(j, ny)+1)   ! up
        end do
    ...
    ! Set or clear a bit at a certain position
    elemental pure function setbit(val, pos, bitval) result(res)
        integer, intent(in) :: val, pos, bitval
        integer :: res
        if (bitval == 1) then
            res = ibset(val, pos)
        else
    ...
    ! morton encoding of two integers (x,y) into one integer z
    elemental pure function morton_encode(x, y) result(z)
        integer, intent(in) :: x, y
        integer :: z, i
        z = 0
        do i = 0, 15
            z = setbit(z, 2*i, ibits(x, i, 1))
            z = setbit(z, 2*i+1, ibits(y, i, 1))
        end do
    end function morton_encode

    ! help function to access the lookup table for coordinate i
    ...
    ! help function to access the lookup table for coordinate j
