...
def smooth_interior(nx1, ny1, dx, dy,
                    etap, etab, vx, vy,
    ...
    for i in nb.prange(1, ny1 - 2):
        for j in range(1, nx1 - 2):
            ...
            vx1, vx2, vx3, vx4, vx5, vy1, vy2, vy3, vy4 = \
                _compute_x_momentum_coefficients(dx, dy, etaA, etaB_vx, eta1_vx, eta2)
            ...
            vy1, vy2, vy3, vy4, vy5, vx1, vx2, vx3, vx4 = \
                _compute_y_momentum_coefficients(dx, dy, etaA, etaB_vy, eta1_vy, eta2)
...
@nb.njit(cache=True)
def smooth_last_row(nx1, ny1, dx, dy,
                    etap, etab, vx, vy,
                    relax_v, vx_rhs, vx_new):
    i = ny1 - 2
    for j in range(1, nx1 - 2):
        etaA = etap[i, j]
        eta2 = etab[i, j]
...
@nb.njit(cache=True)
def smooth_last_column(nx1, ny1, dx, dy,
                       etap, etab, vx, vy,
                       relax_v, vy_rhs, vy_new):
    j = nx1 - 2
    for i in range(1, ny1 - 2):
        etaA = etap[i, j]
        eta2 = etab[i, j]
        etaB_vy = etap[i + 1, j]
        eta1_vy = etab[i, j - 1]
...
@nb.njit(cache=True, parallel=True)
def velocity_smoother(nx1, ny1, dx, dy,
                            etap, etab,
                            vx, vy,
                            relax_v, BC,
                            vx_rhs, vy_rhs,
    ...
    for _ in range(max_iter):
        smooth_interior(nx1, ny1, dx, dy,
                        etap, etab, vx, vy,
                        relax_v, vx_rhs, vy_rhs,
                        vx_new, vy_new)

        smooth_last_row(nx1, ny1, dx, dy,
                        etap, etab, vx, vy,
                        relax_v, vx_rhs, vx_new)

        smooth_last_column(nx1, ny1, dx, dy,
                           etap, etab, vx, vy,
                           relax_v, vy_rhs, vy_new)
