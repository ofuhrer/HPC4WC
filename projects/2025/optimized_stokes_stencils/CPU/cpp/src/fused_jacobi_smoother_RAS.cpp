...
void smoother_micro_kernel(int ii, int jj, int nx1, int ny1, double dx, double dy,
                           const double* etap, const double* etab,
                           const double* vx_rhs, const double* vy_rhs,
                           double* vx_src, double* vy_src,
    ...
    for (int t = 0; t < T_INNER; ++t) {
            ...
            for (int j = jj; j < j_max; ++j) {
                ...
                if (j < nx1 - 2) {
                    ...
                    double sum =
                        ...
                        vy1 * AT(vy_src, i - 1, j, nx1) +
                        vy2 * AT(vy_src, i, j, nx1) +
                        vy3 * AT(vy_src, i - 1, j + 1, nx1) +
                        vy4 * AT(vy_src, i, j + 1, nx1);
                ...
                if (i < ny1 - 2) {
                    ...
                    double sum =
                        ...
                        vx1 * AT(vx_src, i, j - 1, nx1) +
                        vx2 * AT(vx_src, i + 1, j - 1, nx1) +
                        vx3 * AT(vx_src, i, j, nx1) +
                        vx4 * AT(vx_src, i + 1, j, nx1);
        ...
        // Ping-pong
        std::swap(vx_src, vx_dst);
        std::swap(vy_src, vy_dst);
    }
}
    ...
    for (int outer = 0; outer < outer_iters; ++outer) {
        ...
        for (int ii = 1; ii < ny1 - 1; ii += TILE_I) {
            for (int jj = 1; jj < nx1 - 1; jj += TILE_J) {
                smoother_micro_kernel(ii, jj, nx1, ny1, dx, dy,
                                      etap, etab,
                                      vx_rhs, vy_rhs,
                                      vx, vy,
                                      vx_new, vy_new,
