    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        for (int i = 1; i < ny1 - 1; ++i) {
            for (int j = 1; j < nx1 - 1; ++j) {
                ...
                // vx update (x-staggered, skip right edge)
                if (j < nx1 - 2) {
                    double etaB_vx = AT(etap, i, j + 1, nx1);
                    double eta1_vx = AT(etab, i - 1, j, nx1);

                    double vx1, vx2, vx3, vx4, vx5;
                    double vy1, vy2, vy3, vy4;

                    compute_x_coeff(dx, dy, etaA, etaB_vx, eta1_vx, eta2,
                                    vx1, vx2, vx3, vx4, vx5,
                                    vy1, vy2, vy3, vy4);
                    ...
                    double diag = vx3;
                    AT(vx_new, i, j, nx1) = (1.0 - relax_v) * AT(vx, i, j, nx1) + relax_v * (AT(vx_rhs, i, j, nx1) - sum_neighbors) / diag;
                }

                // vy update (y-staggered, skip bottom edge)
                if (i < ny1 - 2) {
                    double etaB_vy = AT(etap, i + 1, j, nx1);
                    double eta1_vy = AT(etab, i, j - 1, nx1);

                    double vy1, vy2, vy3, vy4, vy5;
                    double vx1, vx2, vx3, vx4;

                    compute_y_coeff(dx, dy, etaA, etaB_vy, eta1_vy, eta2,
                                    vy1, vy2, vy3, vy4, vy5,
                                    vx1, vx2, vx3, vx4);
                    ...
                    double diag = vy3;
                    AT(vy_new, i, j, nx1) = (1.0 - relax_v) * AT(vy, i, j, nx1) + relax_v * (AT(vy_rhs, i, j, nx1) - sum_neighbors) / diag;
                }
            }
        }
